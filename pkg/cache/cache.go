...
func (ac *AgentCache) Online(agent entities.LightningMonkeyAgent) {
	...
	ac.Unlock()
	logrus.Debugf("Agent %s online..., etcd-role: %t, master-role: %t, minion-role: %t", agent.HasETCDRole, agent.HasMasterRole, agent.HasMinionRole)
}
...
func (ac *AgentCache) Offline(agent entities.LightningMonkeyAgent) {
	...
	ac.Unlock()
	logrus.Debugf("Agent %s offline..., etcd-role: %t, master-role: %t, minion-role: %t", agent.HasETCDRole, agent.HasMasterRole, agent.HasMinionRole)
}

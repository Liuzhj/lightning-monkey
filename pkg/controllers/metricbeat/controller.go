...
func (dc *MetricbeatDeploymentController) HasInstalled() (bool, error) {
	...
	ds, err := dc.client.CoreClient.ExtensionsV1beta1().DaemonSets("kube-system").Get("metricbeat", v1.GetOptions{})
	if err != nil {
		if k8sErr.IsNotFound(err) {
			return false, nil
		}
		return false, fmt.Errorf("Failed to retrieve DaemonSets(%s/%s) object from given Kubernetes cluster, error: %s", "kube-system", "metricbeat", err.Error())
	}
	if ds != nil {

...
func QueryAgentNextWorkItem(metadataId string) (*entities.AgentJob, error) {
	...
	if agent.HasETCDRole && !agent.HasProvisionedETCD && canDeployETCD == entities.ConditionConfirmed {
		return &entities.AgentJob{Name: entities.AgentJob_Deploy_ETCD, Arguments: map[string]string{"addresses": strings.Join(strategy.GetAgentsAddress(entities.AgentRole_ETCD, entities.AgentStatusFlag_Whatever), ",")}}, nil
	}
	...
	if agent.HasMinionRole && !agent.HasProvisionedMinion && canDeployMinion == entities.ConditionConfirmed {
		masterIps := strategy.GetAgentsAddress(entities.AgentRole_Master, entities.AgentStatusFlag_Provisioned)

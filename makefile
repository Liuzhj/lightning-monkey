timeFlag = $(shell date +%Y%m%d-%H%M%S)
...
build :
	...
	rm -rf img_etcd && rm -rf img_hyperkube && rm -rf img_infra
	echo "Start building APIServer Docker image..."
	docker build -t g0194776/lightning-monkey-apiserver:$(timeFlag) -f Dockerfile.apiserver .
